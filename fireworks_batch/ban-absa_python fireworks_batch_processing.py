...
OUTPUT_CSV = f"ban-absa_dataset_{normalize_model_id_for_batch(MODEL_ID).split('/')[-1]}_fireworks_processed_{RUN_TAG}.csv"
...
def main() -> None:
    ...
    if state not in ("COMPLETED", "JOB_STATE_COMPLETED"):
        ...
        try:
            download_dir = Path(f"ban-absa_fireworks_batch_outputs_{normalize_model_id_for_batch(MODEL_ID).split('/')[-1]}")
    ...
    # Identify results and errors files
    ...
    with results_path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            body = rec.get("response") or rec.get("body")
    ...
    # Step 7: Load original dataset and merge predictions
    df = pd.read_csv(INPUT_CSV).reset_index(drop=True)
    for col in ["fw_raw_text", "fw_status_code", "fw_error", "pred_aspect", "pred_sentiment"]:
        if col not in df.columns:
            df[col] = None

    filled, parse_fail = 0, 0
    for custom_id, payload in results_by_custom_id.items():
        ...
        df.at[row_idx, "fw_status_code"] = payload.get("status_code")
        df.at[row_idx, "fw_error"] = json.dumps(payload.get("error"), ensure_ascii=False) if payload.get("error") else None

        out_text = extract_output_text(payload.get("body"))
        ...
        df.at[row_idx, "pred_aspect"] = parsed_obj.get("aspect")
        df.at[row_idx, "pred_sentiment"] = parsed_obj.get("sentiment")
        filled += 1

    # Save processed CSV
    df.to_csv(OUTPUT_CSV, index=False)
    print(f"Saved processed dataset with Fireworks results to: {OUTPUT_CSV}\nRows filled: {filled}/{len(df)}; parse failures: {parse_fail}")

    # --------------------------------------------
    # Step 8: Evaluate model performance (ABSA metrics)
    # --------------------------------------------
