...
def get_strict_json_schema(model_class: Type[BaseModel]) -> dict:
    ...
    The returned dictionary is ready to be embedded under the
    `text.format` field of a Responses API call.
    ...
    -------
    dict
        A dictionary describing the strict JSON schema.  The top‑level keys
        are `type`, `name`, `schema` and `strict` as required by the
        Responses API structured outputs interface【534535455652949†L3390-L3397】.
    """
...
def build_batch_requests(
    ...
    """
    Generate a JSONL file for the Responses API batch call.
    ...
    output_jsonl : Path
        ...
        created as needed.
    system_instruction_text : str
        The system prompt to insert into the 'instructions' field of each
        request.  This should contain task descriptions, taxonomy definitions
        and emotion cues.
    model_name : str, optional
        Which OpenAI model to call.  Must be the same for all requests in the
        batch (default "gpt-5-mini").
    ...
    with output_jsonl.open("w", encoding="utf-8") as f:
        for idx, row in df.iterrows():
            ...
            # Assemble the body for /v1/responses
            body = {
                "model": model_name,
                "instructions": system_instruction_text,
                "input": user_input,
                "text": {
                    "format": response_format_obj
                },
...
if __name__ == "__main__":
    ...
    3. Determine the overall_emotion expressed in the review.  Use the following emotion classes and their subordinate cues:
       ...
       - Happiness: amusement, bliss, cheerfulness, gaiety, glee, jolliness, joviality, joy, delight, enjoyment, gladness, jubilation, elation, satisfaction, ecstasy, euphoria, enthusiasm, zeal, zest, excitement, thrill, exhilaration, contentment, pleasure, pride, triumph, eagerness, hope, optimism, enthrallment, rapture, relief.
    ...
    Match the predominant emotional cues in the review to these lists and output one of: Love, Happiness, Anger, Fear, or Sadness.
    ...
    input_path = Path("data/processed/dataset_final_selected.csv")
    output_path = Path("batch_tasks_responses_api.jsonl")
